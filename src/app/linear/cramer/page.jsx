...
import { Cramer_func } from "./Cramer_func.js";
...
function Cramer() {
  ...
  function Calculate() {
    ...
    let result = Cramer_func(matrixA, matrixB);
    setAns(result);
  }
  ...
  return (
    <div className="container mt-5 mb-5">
      <div className="text-center mb-5">
        <h1 className="text-3xl font-bold">Cramer's Rules</h1>
      </div>
...
class CramerCalculator {
  constructor(dimension) {
    ...
    this.matrixA = this.createMatrix(dimension);
    this.matrixB = new Array(dimension).fill("");
    this.ans = [];
  }
  ...
  updateMatrixDimension(dimension) {
    if (dimension < 2) {
      alert("Dimension must be >= 2");
      return;
    }
  ...
  updateCellA(row, col, value) {
    this.matrixA[row][col] = parseFloat(value);
  }

  updateCellB(row, value) {
    this.matrixB[row] = parseFloat(value);
  }

  Calculate() {
    let newMatrixA = [...this.matrixA];
    let newMatrixB = [...this.matrixB];

    for (let i = 0; i < this.matrixDimension; i++) {
      for (let j = 0; j < this.matrixDimension; j++) {
        if (newMatrixA[i][j] === "") {
          newMatrixA[i][j] = 0;
        }
      }
    ...
    this.ans = Cramer_func(this.matrixA, this.matrixB);
  }
}
...
function Cramer() {
  ...
  let [calculator, setCalculator] = useState(new CramerCalculator(3));
}
